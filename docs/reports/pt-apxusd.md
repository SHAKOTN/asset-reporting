...
- Fully-doxxed leadership: [Joseph Onorati](https://blockworks.com/speaker/joseph-onorati) (ex-Kraken CSO 2016-2024; CEO of NASDAQ [DFDV](https://www.globenewswire.com/news-release/2026/02/26/3245596/0/en/DeFi-Development-Corp-Announces-Investment-in-Apyx.html)), Dan Kang, [Dawson Reid](https://github.com/apyx-labs), Pete Humiston — all with verifiable press + SEC trail.
- Institutional-grade custody: BitGo qualified custodian plus PCAOB-audited monthly reserve attestations — documented in the [apxUSD overview](https://docs.apyx.fi/product-overview/apxusd-overview).
- All 5 core Apyx contracts verified on Etherscan: [apxUSD proxy](https://etherscan.io/address/0x98A878b1Cd98131B271883B390f68D2c90674665#code), [CR capped feed](https://etherscan.io/address/0x2037a5Eb67aa9B2FBF50042B724D8c4dB80F23b4#code), [CR raw feed](https://etherscan.io/address/0x823210Eb6390B88e2b8ad7152DF5D8F30B8FD305#code), [AccessManager](https://etherscan.io/address/0xe167330E2Eac88666de253e9607C6d9ae0cA2824#code), [MinterV0](https://etherscan.io/address/0x2c36e1adFaA80ee0324B04cC814F5207Bb7Ba76e#code).
- Audit bar met: Quantstamp + Certora + Zellic listed at [docs.apyx.fi/resources/audits](https://docs.apyx.fi/resources/audits) (PDFs gated).
- Cross-chain surface is minimal and safe-by-construction: [Chainlink CCIP with RMN enabled](https://docs.chain.link/ccip/concepts/architecture/key-concepts), Ethereum + Base only.
- Pendle PT layer is standard V2 code — engineering risk concentrates on the apxUSD collateral layer, not the wrapper.
- apxUSD minting is not unbounded EOA: [MinterV0](https://etherscan.io/address/0x2c36e1adFaA80ee0324B04cC814F5207Bb7Ba76e) enforces 10M/day cap gated by [3/6 MAINTAINER Safe](https://etherscan.io/address/0xf9862efc1704ac05e687f66e5cd8c130e5663ce2) + 4h delay.
- Liquidity plumbing adequate at proposed cap: [Curve NG](https://etherscan.io/address/0xe1b96555bbeca40e583bbb41a11c68ca4706a414) + Uni v4 ≈ $34M combined TVL, max-dump slippage 0.15–0.42% at $1-3M.
...
- No public bug-bounty program ([Immunefi](https://immunefi.com) / [Cantina](https://cantina.xyz) search negative) — pulls code-quality down despite 3 audits.
- Aggregators ([Paraswap](https://app.paraswap.io), [Odos](https://app.odos.xyz)) don't route PT-apxUSD as input — liquidator needs a bespoke 3-hop PT→SY→apxUSD→USDC path pre-expiry.
...
*Why*: `apxUSD.mint()` is NOT callable by arbitrary EOAs — it reverts unless the caller holds role 4 in AccessManager. The only address that holds role 4 is the MinterV0 contract. So the full mint path is: (1) 3-of-6 MAINTAINER Safe signers sign an EIP-712 mint order, (2) the Safe calls `MinterV0.requestMint`, (3) MinterV0 validates the order + enforces its own 10M/tx + 10M/day + 300M-supply-cap limits and waits its 4h role-4 execution delay, (4) MinterV0 calls `apxUSD.mint()` which passes the AccessManager role-4 check. Any other caller to `apxUSD.mint()` reverts. The narrow trust floor is 3 non-colluding signers + a bounded rate (10M/day) + a bounded supply cap (300M).

```bash
# apxUSD.mint (selector 0x156e29f6) is gated by role 4
...
# → expect: 4
# Role 4 has a 4h execution delay
cast call 0xe167330E2Eac88666de253e9607C6d9ae0cA2824 'canCall(address,address,bytes4)(bool,uint32)' 0x2c36e1adFaA80ee0324B04cC814F5207Bb7Ba76e 0x98A878b1Cd98131B271883B390f68D2c90674665 0x156e29f6 --rpc-url $RPC
# → expect: (false, 14400)  # role 4, 4h exec delay
# MinterV0 itself is an AccessManaged contract
cast call 0x2c36e1adFaA80ee0324B04cC814F5207Bb7Ba76e 'authority()(address)' --rpc-url $RPC
# → expect: 0xe167330E2Eac88666de253e9607C6d9ae0cA2824
# Non-MinterV0 caller reverts when attempting mint
cast call 0x98A878b1Cd98131B271883B390f68D2c90674665 'mint(address,uint256)' 0x0000000000000000000000000000000000000001 1 --rpc-url $RPC --from 0x0000000000000000000000000000000000000001
# → expect: revert: AccessManagedUnauthorized
```
...
*Why*: Holds role 24 (apxUSD `upgradeToAndCall`, 3-day exec delay), role 21 (pause, 0 delay), role 22 (unpause, 4h delay), role 23 (setDenyList + setSupplyCap, 1-day delay), role 25 (setCCIPAdmin + setAuthority, 7-day delay). 3 of its 6 keys can push an apxUSD upgrade after 3 days, pause immediately, or deny-list after 1 day. Compare to Step 7's ADMIN Safe (4/6) which is a different governance surface.

```bash
# Safe threshold
cast call 0xf9862efc1704ac05e687f66e5cd8c130e5663ce2 'getThreshold()(uint256)' --rpc-url $RPC
# → expect: 3
# Safe owners (compare to ADMIN Safe owners at Step 7)
cast call 0xf9862efc1704ac05e687f66e5cd8c130e5663ce2 'getOwners()(address[])' --rpc-url $RPC
# → expect: 6 EOAs, 5 overlap with ADMIN Safe
# Verify apxUSD upgrade path: 3-day delay enforced
...
# Pause is instant (role 21, 0 delay)
...
### 🔑 Governance & Security  ·  score 3 / confidence 0.88 {: .agent-section .agent-governance-security }

#### <span class="sev-badge sev-critical">[CRITICAL]</span> 4-of-6 ADMIN Safe can swap CR oracle implementation instantly (no timelock) {: .finding-header .finding-critical }
...
#### <span class="sev-badge sev-critical">[CRITICAL]</span> CR oracle feeds stale ~30 days — no on-chain heartbeat, no oracle-operator role {: .finding-header .finding-critical }
...
#### <span class="sev-badge sev-high">[HIGH]</span> ADMIN Safe is only 4-of-6; below Fira's 5/9+ policy for fund-critical surfaces {: .finding-header .finding-high }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> Role 31 (GUARDIAN) has grantDelay = 0 and 7 EOA members co-extensive with ADMIN {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> apxUSD `pause()` is instant (role 21, 3/6 with 0 delay) — DoS vector on Fira liquidations {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> AccessManager self-admin delay = 0 — manager reconfigurable in single tx {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-info">[INFO]</span> Multisig & privileged role inventory (mandatory table) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Role labels + delay posture (reference) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> apxUSD.mint is rate-limited: MinterV0 caps 10M/day + 4h exec delay, upstream gate is 3-of-6 Safe {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> apxUSD upgrade (role 24) gated by 3-of-6 Safe + 3-day delay — low signer bar for $180M supply {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> DenyList + setSupplyCap both gated by same 3-of-6 Safe with 1-day delay {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-low">[LOW]</span> apxUSD pause() is instant-role (3/6 quorum, 0 delay) — DoS lever on Fira liquidation flow {: .finding-header .finding-low }
...
#### <span class="sev-badge sev-low">[LOW]</span> setCCIPAdmin + setAuthority gated by 3-of-6 Safe with 7-day delay (appropriate) {: .finding-header .finding-low }
...
#### <span class="sev-badge sev-info">[INFO]</span> AccessManager ADMIN_ROLE (role 0) held by 4-of-6 Safe, not an EOA {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Both Safes share the same 6 owner keys — effective governance bar is 3-of-6 for all ops {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Role taxonomy: roles 4 + 21-25 are unlabeled (no RoleLabel events) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Fira exposure is indirect — PT-apxUSD, not apxUSD at rest {: .finding-header .finding-info }
...
### 🧪 Code Quality  ·  score 7 / confidence 0.65 {: .agent-section .agent-code-quality }

#### <span class="sev-badge sev-medium">[MEDIUM]</span> No public bug bounty program {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-info">[INFO]</span> All 5 Apyx core contracts verified on Etherscan (Exact Match) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Three audits published (Quantstamp, Certora, Zellic) — all top-tier firms {: .finding-header .finding-info }

Listed at [docs.apyx.fi/resources/audits](https://docs.apyx.fi/resources/audits): Quantstamp (Feb 2026), Certora (Mar 2026), Zellic (Mar 2026). PDFs are not directly linked from the docs index at time of assessment — reader should request them from Apyx. Meets the 2+ audit bar.

*Evidence:* https://docs.apyx.fi/resources/audits

#### <span class="sev-badge sev-info">[INFO]</span> Active repo with Foundry + Slither + invariant tests {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> No known incidents {: .finding-header .finding-info }

No exploits or advisories found via Immunefi, Rekt News, GitHub Security Advisories for Apyx.

#### <span class="sev-badge sev-info">[INFO]</span> Pendle PT layer uses standard Pendle V2 code {: .finding-header .finding-info }
...
### 🌉 Cross-Chain Surface  ·  score 8 / confidence 0.85 {: .agent-section .agent-cross-chain-surface }

#### <span class="sev-badge sev-info">[INFO]</span> apxUSD is live on 2 EVM chains: Ethereum mainnet and Base {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> PT-apxUSD itself is Ethereum-only (Pendle PT, not cross-chain) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> CCIP has RMN (Risk Management Network) enabled — strongest bridge defense available {: .finding-header .finding-info }

Secondary independent DON that monitors and can veto the primary Committing DON. Default on all CCIP lanes today.

#### <span class="sev-badge sev-info">[INFO]</span> Base-side token pool + CCIP admin registry not inspected on-chain in this pass {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> 2 chains via canonical bridge with RMN → score 8 per rubric {: .finding-header .finding-info }
...
### 👥 Team Transparency  ·  score 8 / confidence 0.75 {: .agent-section .agent-team-transparency }

#### <span class="sev-badge sev-info">[INFO]</span> Team is fully doxxed — 4 named leaders, all with verifiable LinkedIn + press trail {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Primary backer = NASDAQ-listed DFDV (first institutional capital, 2026-02-26) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Onorati was interim CEO of CaVirtEx during its 2015 hack and wind-down {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> No rugpulls, scams, insider drama, or sanctioned-jurisdiction ties for any named member {: .finding-header .finding-info }

#### <span class="sev-badge sev-info">[INFO]</span> 'Apyx Labs' corporate entity not publicly disclosed; likely Delaware (unverified) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> GitHub org apyx-labs has no public members; engineering attribution relies on bios, not commits {: .finding-header .finding-info }

Minor opacity — typical for stealth-launch crypto orgs but worth noting given the team is otherwise doxxed.

#### <span class="sev-badge sev-info">[INFO]</span> Multi-sig signer identities not publicly disclosed {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> FLAG_ANON_TEAM_WITH_PRIVILEGE not triggered — team is public, not pseudonymous {: .finding-header .finding-info }


### 💧 Liquidity Depth  ·  score 5 / confidence 0.85 {: .agent-section .agent-liquidity-depth }

#### <span class="sev-badge sev-high">[HIGH]</span> Aggregators do NOT support PT-apxUSD as input — custom liquidator path required {: .finding-header .finding-high }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> Aggregators (Paraswap, Odos) route 100% through Curve — V4 pool is un-routed {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> Concentration risk: ~$34M TVL in 2 pools on one chain; Apyx is POL-heavy {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-info">[INFO]</span> Asset graph: PT-apxUSD → SY-apxUSD → apxUSD (1:1 at maturity) {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Two meaningful apxUSD/USDC pools on mainnet: Curve NG + Uniswap V4 {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Curve slippage ladder (apxUSD→USDC) verified via on-chain get_dy {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Uniswap V4 absorbs $500k–$3M at <0.01%, reverts around $3.5–5M (concentrated-liquidity cliff) {: .finding-header .finding-info }

V4 Quoter used for measurement; V4 ladder is effectively free up to the depth cliff, zero after.

#### <span class="sev-badge sev-info">[INFO]</span> Pendle AMM holds 2.39M PT + 3.64M SY (~$5.98M USD), 58 days to expiry, implied APY 14.71% {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Post-expiry path (2026-06-18+) is safer: PT redeems 1:1 via factory burn, no AMM slippage on PT leg {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> At proposed 88% LLTV and $1–3M cap, max liquidation dump is $880k–$2.64M — absorbed on Curve alone at 0.15–0.42% {: .finding-header .finding-info }

Well under the 3.73% liquidation incentive. Plumbing viable at the proposed cap; thin beyond $5M.


### 🎭 Controversy  ·  score 5 / confidence 0.75 {: .agent-section .agent-controversy }

#### <span class="sev-badge sev-medium">[MEDIUM]</span> Structural similarity to Nov 2025 xUSD/Stream Finance collapse {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-medium">[MEDIUM]</span> CR oracle feed stale 25-30 days; no on-chain heartbeat {: .finding-header .finding-medium }
...
#### <span class="sev-badge sev-info">[INFO]</span> Where does yield come from? {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> DFDV sponsor is the former Janover Inc. — $300M valuation on $3M raise {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> Legitimate team, no tainted flow, no regulatory action {: .finding-header .finding-info }
...
#### <span class="sev-badge sev-info">[INFO]</span> No recent unrecovered exploit on Apyx or direct dependencies {: .finding-header .finding-info }
